...
export default {
  ...
  methods: {
    ...
    playAll() {
      let that = this;
      let curMusic = this.playlist[this.index];
      axios
        .get('https://api.imjad.cn/cloudmusic/?type=song&id=' + curMusic.id)
        .then(response => {
          // console.log('获取url',response.data.data[0].url)
          that.curMusicInfo = {
            id: curMusic.id,
            index: that.index,
            name: curMusic.name,
            albumName: curMusic.album.name,
            imgUrl: curMusic.album.blurPicUrl,
            artists: curMusic.artists[0].name,
            musicUrl: response.data.data[0].url
          };
        }).then(() => {
          that.$store.commit('setMusicInfo', that.curMusicInfo)
        })
        .catch()
    }
  },
